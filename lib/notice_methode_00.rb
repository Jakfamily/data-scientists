...
sorted_string = handle_twiter.sort_by{|str| str.length} # trie le tableau par longueur de chaîne de caractères
puts sorted_string # affiche le tableau trié
...
result = Hash.new { |h, k| h[k] = [] } # |h,k| on cree une pair cles valeur vide  et on ajoute une nouvelle valeur --
# h[k] = [] vide 
...
handle_twiter.each do |string|
  ...
  result[length] << string # << operateur pour ajouter un element string a la fin du tableau 
end
