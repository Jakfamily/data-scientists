...
def menu(handle_twitter)
    loop do
      puts "Choisissez l'ordre dans lequel vous souhaitez voir les réponses :"
      ...
      when 1
        puts " 1.Combien y a-t-il de handle dans cette array ?"
        puts ""
        puts "il y a exactement #{handle_twitter.length} handle twiter"
        puts ""
        
      when 2
        puts " 2. Quel est le handle le plus court de cette liste ?"
        puts ""
        short_string = handle_twitter.select{|str| str.length <5}# on selectione les string <5 avec la methode select
      ...
      when 3
        puts " 3. Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)"
        puts ""
        long_string = handle_twitter.select{ |str| str.gsub('@', '').length == 5 } # on selectione les string == 5 en
      ...
      when 4
        puts "4. Combien commencent par une majuscule (première lettre juste après le @) ?"
        puts ""
        count = handle_twitter.select {|str| str =~ /@([A-Z])/}.count
      ...
      when 6
        puts "6. Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)"
        puts ""
        sorted_string = handle_twitter.sort_by{|str| str.length}
        puts sorted_string
      ...
      when 7
        puts "7. Quelle est la position dans l'array de la personne @epenser ?"
        puts ""
        handle_twitter.include?("@epenser") # include? permet de chercher une string dans un arr 
        index_in_array = handle_twitter.index("@epenser") # index permet d'afficher le numero d'index du mots rechercher 
